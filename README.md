...
```
A role-based university management system built with **ASP.NET Core Web API**,
**Entity Framework Core**, and **SQL Server**. It supports three roles —
**Admin**, **Professor**, and **Student** — with features for managing
departments, majors, courses, sections, enrollments, marks, attendance,
notifications, and special requests.
```
...
- **API Testing:** Postman (Swashbuckle 10.2.3 is referenced, but Swagger UI is
not configured in `Program.cs`)
```
...
```
> **Note:** The SQL script does not include seed data for `EnrollmentStatus`,
`SpecialRequestStatus`, `SpecialRequestType`, or an initial Admin user. You must
insert these manually before using the API.
```
...
```
| GET | `api/admin/get-all-special-requests?statusId=` | List special requests
by status |
```
...
```json
{
  "ConnectionStrings": {
    "DefaultConnection":
"Server=localhost;Database=Uni_App_Db;Trusted_Connection=True;TrustServerCertifi
cate=True;"
  },
  "Jwt": {
...
```
Run the provided SQL script against your SQL Server instance to create the
schema.
```
...
```
Edit `appsettings.Development.json` with your SQL Server connection and a secure
JWT key.
```
...
```
> Swagger/Swashbuckle packages are referenced, but Swagger UI is not enabled in
`Program.cs`. If you want Swagger, add `AddSwaggerGen()` and
...
- ``Program.cs` — DI, JWT, DbContext, service registration - `UniAppDbContext.cs` — EF Core mappings` 
...
- ``Models/` — EF Core entities - `Enums/` — EnrollmentStatusEnum, SpecialRequestStatusEnum, SpecialRequestTypeEnum` 
